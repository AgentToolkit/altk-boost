...
def all_testcase_postprocess(
    ...
    if (
        len(
            set(transformed_tool_spec["operation"][operation_type].keys()).intersection(
        ...
        for param_type in ["request_body_params", "query_params", "path_params"]:
            for key in transformed_tool_spec["operation"][operation_type][param_type]:
                if (
                    transformed_tool_spec["operation"][operation_type][param_type][key][
                        "required"
                    ]
                    == "True"
                ):
        ...
        for key in transformed_tool_spec["operation"][operation_type]:
            if (
                transformed_tool_spec["operation"][operation_type][key]["required"]
                == "True"
            ):
                required_keys.append(key)
    ...
    if (
        ...
        ).intersection(set(["path_prams", "query_params", "request_body_params"]))
        overlapping_params = set()
        for cm_key in common_keys:
            if cm_key == "query_params" or cm_key == "path_params":
                overlapping_params = overlapping_params.union(
                    set(
        ...
        if "request_body_params" in list(
            transformed_tool_spec["operation"][operation_type].keys()
        ):
    ...
    for testcase_index in positive_testcases:
        ...
        if "__requestBody__" in testcase:
            for param in testcase["__requestBody__"]:
                if (
                    param
                    in transformed_tool_spec["operation"][operation_type][
                        "request_body_params"
                    ]
                ):
                    if (
                        ...
                        if param not in overlapping_params:
                            if (
                                param not in mandatory_parameters_initial
                                and param
                                not in number_of_mandatory_parameters_covered_in_request_body_list
                            ):
                                number_of_mandatory_parameters_covered_in_request_body_list.append(
                                    param
                                )
                        else:
                            if (
                    ...
                    if param not in overlapping_params:
                        if (
                            param not in total_param_covered
                            and param
                            not in number_of_total_parameters_covered_in_request_body_list
                        ):
                            number_of_total_parameters_covered_in_request_body_list.append(
                                param
                            )
                    else:
                        if (
    ...
    number_of_mandatory_parameters_covered = (
        number_of_mandatory_parameters_covered
        + len(number_of_mandatory_parameters_covered_in_request_body_list)
    )
    if "__requestBody__" in number_of_total_parameters_covered_in_request_body_list:
        total_param_covered_number = (
            total_param_covered_number
            + len(number_of_total_parameters_covered_in_request_body_list)
            - 1
        )
    else:
        total_param_covered_number = total_param_covered_number + len(
            number_of_total_parameters_covered_in_request_body_list
        )
    number_of_optional_parameters_covered = (
        total_param_covered_number - number_of_mandatory_parameters_covered
    )
    ...
    for testcase_index_outer in [positive_testcases, negative_testcases]:
        if testcase_index_outer is not None:
            for testcase_index in testcase_index_outer:
                ...
                if "__requestBody__" in list(testcase.keys()):
                    test_case_specific_details["mandatory_params"] = list(
                        set(testcase.keys()).intersection(set(required_keys))
                    ) + list(
                        set(
                            number_of_mandatory_parameters_covered_in_request_body_list
                        ).intersection(set(testcase["__requestBody__"].keys()))
                    )
                else:
                    test_case_specific_details["mandatory_params"] = list(
                        set(testcase.keys()).intersection(set(required_keys))
                ...
                for param in testcase:
                    ...
                    if (
                        ...
                        for param_type in [
                            ...
                            for specific_param in transformed_tool_spec["operation"][
                                operation_type
                            ][param_type]:
                                if key == specific_param:
                                    param_type_val = transformed_tool_spec["operation"][
                                        operation_type
                                    ][param_type][specific_param]["type"]
                                    param_found_flag = True
                                    break
                            if param_found_flag:
                                break
                        if key != "__requestBody__" and param_found_flag:
                            input_string = (
                                input_string
                                + str(key)
                                + "(("
                        ...
                        if key == "__requestBody__":
                            for parameters in testcase["__requestBody__"]:
                                ...
                                for param_type in ["request_body_params"]:
                                    for specific_param in transformed_tool_spec[
                                        "operation"
                                    ][operation_type][param_type]:
                                        if parameters == specific_param:
                                            param_type_val = transformed_tool_spec[
                                                "operation"
                                            ][operation_type][param_type][
                                                specific_param
                                            ][
                ...
                if (
                    ...
                    test_case_specific_details["input_parameters"] = (
                        generated_testcase_to_nl_template(testcase)
                    )
                    test_scenario.append(test_case_specific_details)
                else:
                    if testcase_index_outer is positive_testcases:
                        non_hallucinated_keys = set(
                            generated_testcase_to_nl_template(testcase).keys()
                        ).intersection(
                            set(
                                transformed_tool_spec["operation"][
                                    operation_type
                        ...
                        test_case_specific_details["input_parameters"] = {
                            k: generated_testcase_to_nl_template(testcase)[k]
                            for k in non_hallucinated_keys
                        }
    ...
    final_report["Number_of_mandatory_parameters_covered"] = (
        number_of_mandatory_parameters_covered
    )
    final_report["Number_of_optional_parameters_covered"] = (
        number_of_optional_parameters_covered
    )
    final_report["Number_of_positive_test_scenarios"] = (
        number_of_positive_test_scenarios
    )
    final_report["llm_model_details"] = dict()
    try:
        final_report["llm_model_details"]["tcg-model-id"] = client.model_id
    except Exception:
        final_report["llm_model_details"]["tcg-model-id"] = None
    final_report["llm_model_details"]["nlg-model-id"] = None
    final_report["llm_platform_details"] = dict()
    final_report["llm_platform_details"]["tcg-platform"] = None
    ...
    if "python" not in original_tool_spec["binding"]:
        final_report["Skill_operation"] = (
            "API - "
            + original_tool_spec["binding"]["openapi"]["http_path"]
            + " , Operation - "
            + original_tool_spec["binding"]["openapi"]["http_method"]
        )
    else:
        final_report["Skill_operation"] = (
            "python -"
            + original_tool_spec["name"]
            + " , function - "
            + original_tool_spec["binding"]["python"]["function"]
        )
