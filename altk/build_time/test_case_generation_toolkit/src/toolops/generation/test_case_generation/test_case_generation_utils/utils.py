...
logger = logging.getLogger(
    "toolops.validation.test_case_generation_utils.utils.tool_spec_post_process"
)
...
def tool_spec_post_process(tool_spec):
    ...
    try:
        ...
        if "python" in tool_spec_json["binding"]:
            processed_json["operation"][
                tool_spec_json["binding"]["python"]["function"]
            ] = dict()
            for element in tool_spec_json["input_schema"]["properties"]:
                ...
                if (
                    "description"
                    in tool_spec_json["input_schema"]["properties"][element]
                ):
                    title = tool_spec_json["input_schema"]["properties"][element][
                        "description"
                    ]
            ...
            processed_json["operation"][
                tool_spec_json["binding"]["openapi"]["http_method"]
            ] = dict()
            for param_type in [
                "path_params",
                "request_body_params",
            ...
            if "properties" in tool_spec_json["input_schema"]:
                for element in tool_spec_json["input_schema"]["properties"]:
                    input_type = tool_spec_json["input_schema"]["properties"][element][
                        "in"
                    ]
                    if input_type != "header":
                        ...
                        if (
                            ...
                            title = tool_spec_json["input_schema"]["properties"][
                                element
                            ]["description"]
                        if (
                            "type"
                            ...
                            type = tool_spec_json["input_schema"]["properties"][
                                element
                            ]["type"]
                        processed_json["operation"][
                            tool_spec_json["binding"]["openapi"]["http_method"]
            ...
            if (
                "__requestBody__" in tool_spec_json["input_schema"]
                and "properties" in tool_spec_json["input_schema"]["__requestBody__"]
            ):
                for element in tool_spec_json["input_schema"]["__requestBody__"][
                    "properties"
                ]:
                    ...
                    if (
                        "example"
                        in tool_spec_json["input_schema"]["__requestBody__"][
                            "properties"
                        ][element]
                    ):
                        example = tool_spec_json["input_schema"]["__requestBody__"][
                            "properties"
                        ][element]["example"]
                    required_flag = "False"
                    if "required" in tool_spec_json["input_schema"]["__requestBody__"]:
                        ...
                        if (
                            "__requestBody__"
                            in tool_spec_json["input_schema"]["required"]
                        ):
                            required_flag = "True"
                    title = ""
                    type = "Not Specified"
                    if (
                        ...
                        type = tool_spec_json["input_schema"]["__requestBody__"][
                            "properties"
                        ][element]["type"]
                    if (
                        "description"
                        ...
                        title = tool_spec_json["input_schema"]["__requestBody__"][
                            "properties"
                        ][element]["description"]
                    elif (
                        "title"
