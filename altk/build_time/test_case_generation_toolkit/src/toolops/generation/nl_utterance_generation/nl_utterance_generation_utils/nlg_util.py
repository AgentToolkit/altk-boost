...
logger = logging.getLogger(
    "toolops.generation.nl_utterance_generation.nl_utterance_generation_util.nlg_util"
)
...
def rephrase_nl_query(
    ...
    gen_mode,
    clean_nl_utterances,
    max_nl_utterances_per_testcase,
):
    prompt = (
        preamble
        + str(tool_spec)
    ...
    result = execute_prompt(prompt, client, gen_mode, parameters=parameters)
    # print(result)
    result = result.split("\n")
...
def get_nl_query(tool_definition, input_parameter):
    ...
    nl_query = nl_query.rstrip(" when ")  # noqa: B005
