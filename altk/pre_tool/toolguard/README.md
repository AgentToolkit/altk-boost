...
The input of the build phase is a `ToolGuardCodeBuildInput` object, containing:
  ...
  * `out_dir: str`: A directory in the local file system where the ToolGuard objects will be saved. 

The output of the build phase is a `ToolGuardsCodeGenerationResult` object with:
  * `out_dir: str`: Path to the file system where the results were saved. It is the same as the `input.out_dir`.
...
The input of the runtime phase is a `ToolGuardCodeRunInput` object:
  * `generated_guard_dir: str`: Path in the local file system where the generated guard Python code (The code that was generated during the build time, described above) is located.
